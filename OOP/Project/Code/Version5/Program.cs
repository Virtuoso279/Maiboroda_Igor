...
namespace Project
{
    class Program
    {
        static void Main()
        {
            ...
            Console.WriteLine("Проект №46. Версія 5");
        ...
        static void menu()
        {
            ...
            Subject OOP = new Subject("Вступ до ООП", "Бичков О. С.", numberLessons);
            OOP.printSubject();
            if (semester == 1)
            {
            ...
            Console.WriteLine();
            Group IPZ11 = new Group("ФІТ", "ІПЗ", 1, "ІПЗ-11", 25);
            ...
            Points[] marks = new Points[25];
            Attendance[] attending = new Attendance[25];
            for (int i = 0; i < 25; i++)
            {
                marks[i] = new Points(numberLabWorks, numberHomeWorks);
                attending[i] = new Attendance(numberLessons);
                IPZ11[i] = new Student(marks[i], attending[i], fullnames[i]);
            }
